...
topics: ["zsh", "vim", "ohmyzsh", "terminal"]
published: false
---
...
fishいいよね、abbrとりあえず設定しよになったのですが、こんな文言を見つけてしまいました

> vi モードが分かりやすい

なにこれ？
研修やらなんやらでzshだとemacsのキーバインド使えるんですよーということだけを聞いていたのでまさかviキーバインドに対応しているとは考えもしませんでした。
...
つまり`bindkey -v`は`viins`（vi insert mode）キーマップを`main`にリンクするコマンドということですわね。
ここまででvi-modeの入力ができるようになったので実際にどのような操作ができるのかを見ていきます。
...
このセクションは全てzsh独自です。(historyという概念自体がvimにはないので当然ではありますが...)
...
- `accept-line`（`^J`/`^M`）— vimでは単なる改行ですが、zshではコマンドを実行します
- `execute-named-cmd`（`:`）— vimの`:`はExコマンドですが、zshではZLEのウィジェット名を入力して実行する機能です
- `pound-insert`（`#`）— 行頭に`#`をつけてコメント化した上で実行します。コマンドを履歴に残しつつ実行しないためのテクニックとして使えます
- `redisplay`（`^R`）— 画面の再描画。vimでは`^R`はredoですが、zshのvicmdでは`^R`にredisplayとredoの両方がバインドされています
...
`select-a-shell-word`（`aa`）/ `select-in-shell-word`（`ia`）はvimには存在しないzsh独自のテキストオブジェクトです。シェル引数（クォートされた文字列を含む）を単位として選択できます。
